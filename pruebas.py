...
from scripts.bcv_estadisticas_tasas import actulizar_file_tasas
...
# conexion = ConexionBD(base_de_datos="DPANA_A")
...
conexion = DatabaseConnector(sqlserver_connector)
...
# LibroCompraVenta(conexion).libro_ventas(
#     fecha_d="20260512", fecha_h="20260512"
# ).to_excel("libro_ventas.xlsx")
# oNotas_x_agregar = NotasEntregaACxc(conexion).set_documentos_notas_entrega_profit()
# print(oNotas_x_agregar)
oDevoluciones = Devoluciones(conexion)
oDevoluciones.resumen_pedidos_dev(anio="all", mes="all").to_excel(
    "resumen_pedidos_dev.xlsx"
)
...
# EstadoCuentaRutero(conexion).calculo_ganacia_por_factura_comercio_historica().to_excel('calculo_ganacia_por_factura_comercio_historica.xlsx')
