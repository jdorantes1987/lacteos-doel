...
class DatosProfit:
    ...
    def get_identificador_unicos(self, df, name_field) -> DataFrame:
        ...
        df["identificador"] = df.apply(
            lambda x: str(x[name_field]) + x["correl"], axis=1
        )
        return df
    ...
    def data_documentos(self, **kwargs):
        ...
        where_anio = f"" if anio == "all" else f"AND year(docv.fec_reg)='{anio}'"
        where_all = (
            f"WHERE docv.co_tipo_doc='{tipo_doc}' AND docv.anulado=0 " + where_anio
        )
    ...
    def ventas_con_detalle(self, **kwargs):
        ...
        merge_data["saldo_total_doc"] = merge_data.apply(
            lambda x: (
                ...
                else x["saldo_total_doc"] if x["reng_num"] == 1 else 0
            ),
            axis=1,
        )
    ...
    def ventas_sin_detalle(self, **kwargs):
        ...
        lista_cifras = ["monto_base_item", "iva", "total_item", "saldo_total_doc"]
        """
    ...
    def variacion_tasa_en_cobros(self, **kwargs):
        ...
        print(
            "\n" * 1,
            "Diferencia total$= ",
            round(data_sort["variacion$"].sum(), ndigits=2),
        )
    ...
    def facturacion_x_anio(self, **kwargs):
        ...
        doc_g2 = (
            doc_leg.groupby(["anio", "mes"])[["monto_base_item", "total_item"]]
            .sum()
            .reset_index()
        )
        return doc_g2.sort_values(
    ...
    def facturacion_x_anio_vend(self, **kwargs):
        anio, mes, vend, conv_usd = (
            kwargs.get("anio", "all"),
            kwargs.get("mes", "all"),
            kwargs.get("vendedor", "all"),
            kwargs.get("usd", True),
        )
        doc = self.ventas_con_detalle(anio=anio, mes=mes, usd=conv_usd).copy()
        # Listado de tipos de Documentos que deben ser tomados en cuenta para el calculo del impuesto CONATEL
        ...
        if vend == "all":
            ...
            doc_leg = doc[
                doc["co_tipo_doc"].isin(docs_tipos) & (doc["co_tipo_doc"] == vend)
            ]

        doc_leg["anio"] = doc_leg["fec_reg"].dt.year  # Obtiene el año
        doc_leg["mes"] = doc_leg["fec_reg"].dt.month  # Obtiene el mes
        doc_g2 = (
            doc_leg.groupby(["anio", "mes", "co_ven"])[
                ["monto_base_item", "total_item"]
            ]
            .sum()
            .reset_index()
        )
    ...
    def get_id_movbanco(self):
        df = get_read_sql("Select mov_num from saMovimientoBanco " "where origen='BAN'")
        mb = df["mov_num"].max()
    ...
    def cxc_clientes_resum_pivot(self, **kwargs):
        ...
        vendedor = kwargs.get("vendedor", "all")
        df_fact = self.facturacion_saldo_x_clientes_detallado(
    ...
    def dicc_ventas_total_por_anio(self, **kwargs):
        ...
        df_fact = (
            self.facturacion_x_anio(usd=conv_usd)
            .groupby("anio")[["monto_base_item"]]
            .sum()
            .reset_index()
        )
    ...
    def dicc_ventas_total_por_anio_vendedor(self, **kwargs):
        anio, vend, conv_usd = (
            ...
            kwargs.get("vendedor", "0001"),
            kwargs.get("usd", False),
        )
        df_fact = (
            self.facturacion_x_anio_vend(usd=conv_usd)
            .groupby(["anio", "co_ven"])[["monto_base_item"]]
            .sum()
            .reset_index()
        )
        ...
        if vend == "Todos":
            ...
            try:
                ventas_anio_vend = round(
                    df_fact.loc[(anio, vend), "monto_base_item"], ndigits=2
                )
            except KeyError:
