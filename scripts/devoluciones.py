...
class Devoluciones:
    ...
    def resumen_pedidos_dev(self, anio, mes):
        ...
        return df1[
            df1["campo8"].str.contains("DEV")
        ]  # Extrae todos los pedidos por asignar devolucion
    ...
    def datos_devolucion(self, anio, mes):
        ...
        if len(pedido_x_rutero) > 0:
            articulos = DatosProfit(self.conexion).articulos_profit()[
                ["co_art", "art_des", "tipo_imp"]
            ]
            set_numeros_pedidos = set(pedido_x_rutero["doc_num"])
            pedido_detalle = self.pedido_ventas.data_pedido_con_detalle(
                anio=anio, mes=mes
            )
            pedidos_filtrados = pedido_detalle[
                pedido_detalle["doc_num"].isin(set_numeros_pedidos)
            ][
                [
                    "co_tran",
                    "co_art",
            ...
            pedidos_filtrados = (
                ...
                .agg({"total_art": "sum", "monto_base_item": "sum"})
                .reset_index()
            )
            precios_ultimas_notas_entrega = self.notas_entrega.ultimos_precios_notas(
                anio=anio, mes=mes
            )[["co_cli", "co_art", "doc_num", "prec_vta_uni"]]
            pedidos_con_precio2 = merge(
                pedidos_filtrados,
                precios_ultimas_notas_entrega,
                how="left",
            ...
            articulos_precio2 = articulos_precio[articulos_precio["co_precio"] == "02"][
                ["co_art", "co_precio", "monto"]
            ]
            articulos_precio2.set_index(["co_art"], inplace=True)
            pedidos_con_precio2["co_cli"] = pedidos_con_precio2.apply(
                lambda x: x["co_tran"] if x["co_cli"] is nan else x["co_cli"], axis=1
            )
            pedidos_con_precio2["doc_num"] = pedidos_con_precio2.apply(
                lambda x: "s/despacho" if x["doc_num"] is nan else x["doc_num"], axis=1
            )
            pedidos_con_precio2["prec_vta_uni"] = pedidos_con_precio2.apply(
                lambda x: (
            ...
            pedidos_con_precio2["base_precio_2"] = pedidos_con_precio2.apply(
                lambda x: (
            ...
            pedidos_con_precio2["base_precio_2_uni"] = pedidos_con_precio2.apply(
                lambda x: x["base_precio_2"] / x["total_art"], axis=1
            )
            # se incluye el iva de acuerdo a cada producto
            merge_pedidos_art_profit = merge(
                pedidos_con_precio2,
                articulos,
                how="left",
                left_on=["co_art"],
            ...
            merge_pedidos_art_profit["iva_precio_dos"] = merge_pedidos_art_profit.apply(
                lambda x: (
                    (x["base_precio_2"] * 16 / 100) if x["tipo_imp"] == "1" else 0
                ),
                axis=1,
            )
            merge_pedidos_art_profit["total_item_precio_dos"] = (
                merge_pedidos_art_profit["iva_precio_dos"]
                + merge_pedidos_art_profit["base_precio_2"]
            )
            merge_pedidos_art_profit["ganancia"] = merge_pedidos_art_profit.apply(
    ...
    def procesar_devolucion(self, anio, mes):
        ...
        ruteros = (
            pedidos_devolucion["co_tran"]
            .value_counts()
            .reset_index()
            .sort_values(by="co_tran", ascending=True)
        )
        ruteros = ruteros.reset_index(drop=True)
        for i in range(len(ruteros)):
            rutero = ruteros.loc[i, "co_tran"]
            pedidos_devolucion_rutero = pedidos_devolucion[
                pedidos_devolucion["co_tran"] == rutero
            ]
            numeros_pedidos = set(pedidos_devolucion_rutero["doc_num"])
            datos_dev = self.datos_devolucion_x_rutero(
                anio=anio, mes=mes, rutero=rutero
            )
            total_nota_entrega = round(
                datos_dev["total_item_precio_dos"].sum(), ndigits=2
            )
            total_monto_bruto = round(datos_dev["base_precio_2"].sum(), ndigits=2)
            total_iva = round(datos_dev["iva_precio_dos"].sum(), ndigits=2)
            numero_dev = (
                int(
                    str(self.consultas_generales.get_last_number_devol()).replace(
                        "DV", ""
                    )
                )
                + 1
            ...
            self.pedido_ventas.asociar_devolucion_pedido(
                cursor=self.cursor,
