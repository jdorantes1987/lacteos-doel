...
class Cobros:
    ...
    def view_cobros_x_cliente(self, **kwargs):
        ...
        condicion_tipo_cliente = (cobros_doc_cli['tip_cli']==cobros_doc_cli['tip_cli']) if tip_cli == 'all' else (cobros_doc_cli['tip_cli']==tip_cli)
        cobros_doc_cli = cobros_doc_cli[condicion_tipo_cliente]
    ...
    def saldo_anterior_view_cobros_x_cliente(self, **kwargs):
        ...
        cobros = self.view_cobros_x_cliente(tip_cli='R', fecha_d='20010101', fecha_h=fecha_h)
        cobros = cobros.groupby(['co_cli']).agg({'cargo':'sum'}).reset_index()
        cobros['fecha_anterior_a'] = datetime.strptime(fecha_d, '%Y%m%d')
        cobros.rename(columns={'cargo':'sa_cobros'}, inplace=True)
        return cobros[['fecha_anterior_a', 'co_cli', 'sa_cobros']]