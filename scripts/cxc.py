...
class CXC:
    ...
    def view_cxc(self):
        ...
        cxc["saldo"] = cxc.apply(
            lambda x: (
                ...
                if (
                    x["co_tipo_doc"] != "AJNM"
                    and x["co_tipo_doc"] != "AJNA"
                    and x["co_tipo_doc"] != "N/CR"
                )
                else -x["saldo"]
            ),
            axis=1,
        ...
        merge_cxc_clientes = merge(
            cxc, clientes, how="left", left_on=["co_cli"], right_on=["co_cli"]
        )
        return merge_cxc_clientes.sort_values(
    ...
    def cxc_clientes_resum_pivot(self, tipo_cliente):
        ...
        df_cxc["mes"] = df_cxc["fec_emis"].dt.month_name(locale="es_ES.UTF-8").str[:3]
