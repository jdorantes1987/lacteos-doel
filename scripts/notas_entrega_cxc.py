...
class NotasEntregaACxc:
    ...
    def set_documentos_notas_entrega_profit(self):
        return set(
            self.oData.data_documentos(anio="all", mes="all", tipo_doc="NENT")[
                "doc_num"
            ]
        )
    ...
    def set_numeros_notas_entrega_por_agregar(self):
        return (
            self.set_numeros_notas_entrega_profit()
            - self.set_documentos_notas_entrega_profit()
        )
    ...
    def df_notas_de_entrega_por_agregar(self):
        ...
        return encabezado_notas_de_entrega[
            encabezado_notas_de_entrega["doc_num"].isin(
                numeros_notas_entrega_por_agregar
            )
        ]
    ...
    def agregar_notas_entrega_a_cxc(self):
        ...
        for index, row in notas_entrega_por_agregar.iterrows():
            self.add_doc.exe_sql_insert_doc(
                "NENT",
                row["doc_num"],
                row["descrip"],
                row["co_cli"],
