...
st.set_page_config(page_title="Ganancias Ruteros", layout="wide", page_icon="⚡")
...
def data_ganacia_x_facturas(**kwargs):
    ganancias_aplicadas = set(
        Ajustes(conexion).ganancias_aplicadas(**kwargs)["nro_orig"]
    )
    ganancias_hist = (
        estado_cuenta_rutero.calculo_ganacia_por_factura_comercio_historica(**kwargs)
    )
    ganancias_hist = ganancias_hist[
        ~ganancias_hist["doc_num"].isin(ganancias_aplicadas)
    ]
    ganancias_hist["fec_emis"] = ganancias_hist["fec_emis"].dt.normalize()
    group_ganancias_hist = (
        ganancias_hist.groupby(
            ["co_tran", "co_cli", "cli_des", "doc_num", "doc_num_dev", "fec_emis"]
        )
        .agg({"ganancia": "sum"})
        .reset_index()
    )
...
st.caption(
    """Módulo para aplicar ganancia a Ruteros una vez realizado el :orange[cobro total] de cada factura de comercios."""
)
data_ganancia_x_factura = data_ganacia_x_facturas()[
    ["co_tran", "co_cli", "cli_des", "doc_num", "fec_emis", "doc_num_dev", "ganancia"]
]
data_ganancia_x_factura.insert(0, "sel", False)
cmap = plt.colormaps["summer"]
# resumen_movimientos.style.format({"saldo": "${:,.2f}"})
df_style = data_ganancia_x_factura.style.format(
    {"ganancia": "{:,.2f}"}, precision=2
).background_gradient(cmap=cmap, subset=["ganancia"], axis=None)
editor_resumen_mov = st.data_editor(
    ...
    column_config={
        ...
        "fec_emis": st.column_config.DateColumn(
            "fecha", width="small", format="DD-MM-YYYY"
        ),
    },
...
if len(selected_rows) > 0:
    mov_x_aplicar = st.dataframe(
        selected_rows, use_container_width=True, hide_index=True
    )
    ...
    if st.button("Aplicar ganancia"):
        ...
        for index, row in selected_rows.iterrows():
            ...
            agregar_doc.exe_sql_insert_doc(
                "AJNM",
                num_ajnm,
