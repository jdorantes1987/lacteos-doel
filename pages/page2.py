...
st.set_page_config(page_title="Notas de Entrega", layout="wide", page_icon="⚡")
...
def notas_entrega_por_agregar():
    return notas.df_notas_de_entrega_por_agregar()[
        [
            "fec_emis",
            "doc_num",
            "co_cli",
...
if len(st.session_state.notas):
    ...
    st.session_state.notas["total_bruto"] = st.session_state.notas["total_bruto"].apply(
        "{:,.2f}".format
    )
    st.session_state.notas["monto_imp"] = st.session_state.notas["monto_imp"].apply(
        "{:,.2f}".format
    )
    st.session_state.notas["total_neto"] = st.session_state.notas["total_neto"].apply(
        "{:,.2f}".format
    )
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "fec_emis": st.column_config.DateColumn("fecha", format="DD-MM-YYYY")
        },
    )
    if st.button("agregar a Cxc"):
        user_i = st.session_state.get("usuario", "999")
        notas.agregar_notas_entrega_a_cxc(user_i)
        st.cache_data.clear()
