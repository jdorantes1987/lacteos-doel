...
st.set_page_config(page_title="Edo. Cta. Rutero", layout="wide", page_icon="⚡")
...
def mov_facturas_comercios(**kwargs):
    mov_fact_comercio = estado_cuenta_rutero.calculo_ganacia_por_factura_comercio(
        **kwargs
    )
    if len(mov_fact_comercio) > 0:
        mov_fact_comercio = mov_fact_comercio.sort_values(
            by=["fec_emis"], ascending=[False]
        )
        mov_fact_comercio["fec_emis"] = mov_fact_comercio["fec_emis"].dt.strftime(
            "%d-%m-%Y"
        )
...
def movimientos_ganancias_aplicadas(**kwargs):
    ...
    if len(ganancias_aplicadas) > 0:
        ganancias_aplicadas = ganancias_aplicadas.sort_values(
            by=["fec_emis"], ascending=[False]
        )
        ganancias_aplicadas["fec_emis"] = ganancias_aplicadas["fec_emis"].dt.strftime(
            "%d-%m-%Y"
        )
...
with col1:
    ...
    fecha_ini = st.date_input("fecha inicial", value=current_fecha_ini).strftime(
        "%Y%m%d"
    )  # Convierte la fecha a YYYYMMDD
with col2:
    fecha_fin = st.date_input("fecha final").strftime(
        "%Y%m%d"
    )  # Convierte la fecha a YYYYMMDD

mov_ruta = resumen_movimientos_cuenta_ruteros(
    tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
)[
    [
        "co_cli",
        "cli_des",
...
with col:
    st.metric(
        label="Total saldos a la fecha", value="{:,.2f}".format(mov_ruta["saldo"].sum())
    )
...
with tab1:
    ...
    df_style = resumen_movimientos.style.format(
        {"saldo": "{:,.2f}", "sa_total": "{:,.2f}"}, precision=2
    ).background_gradient(cmap=cmap, subset=["saldo", "sa_total"], axis=None)
    editor_resumen_mov = st.data_editor(
        df_style,
        column_config={
            ...
            "total_ne": st.column_config.NumberColumn(
                "notas e.", width="small", help="Despachos realizados a la ruta."
            ),
            "total_fd": st.column_config.NumberColumn(
                "f. directa", width="small", help="Facturación directa de la ruta."
            ),
            "total_fcp2": st.column_config.NumberColumn(
                "f. comercios", width="small", help="Facturación aplicada a la ruta."
            ),
            ...
            "total_fondo": st.column_config.NumberColumn(
                "fondo g.", width="small", help="Fondo de garantía de la ruta."
            ),
        },
    ...
    if len(selected_rows) > 0:
        ...
        st.session_state.rutero_selected_saldo_anterior = selected_rows.iloc[
            0, 3
        ]  # Saldo anterior
        st.session_state.rutero_selected_ne = selected_rows.iloc[
            0, 4
        ]  # Notas de entregar
        st.session_state.rutero_selected_fd = selected_rows.iloc[
            0, 5
        ]  # Facturas directas
        st.session_state.rutero_selected_fcp2 = selected_rows.iloc[
            0, 6
        ]  # Facturas Comercios
...
if len(selected_rows) > 0:
    with tab2:
        ...
        movimiento_x_dia = movimiento_cuenta_rutero_x_dia(
            tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
        )
        ...
        if len(movimiento_x_dia) > 0:
            mov_filtrados_x_rutero = movimiento_x_dia[
                movimiento_x_dia["co_cli"].isin(selected_rows["co_cli"])
            ].copy()
            mov_filtrados_x_rutero["saldo"] = (
                mov_filtrados_x_rutero["total_ne"]
                + mov_filtrados_x_rutero["total_fd"]
                - mov_filtrados_x_rutero["total_fcp2"]
                + mov_filtrados_x_rutero["total_ajust"]
                - mov_filtrados_x_rutero["total_cobro"]
                - mov_filtrados_x_rutero["total_ganancia"]
                - mov_filtrados_x_rutero["total_fondo"]
            )
            mov_filtrados_x_rutero["fec_emis"] = mov_filtrados_x_rutero[
                "fec_emis"
            ].dt.strftime("%d-%m-%Y")
            valores_anterior = []
            valores_anterior.append(st.session_state.rutero_selected)
            valores_anterior.append(
                st.session_state.rutero_selected_name + " (S. Anterior)"
            )
            fecha_anterior = (
                datetime.strptime(fecha_ini, "%Y%m%d") + timedelta(days=-1)
            ).strftime("%d-%m-%Y")
            valores_anterior.append(fecha_anterior)
            valores_anterior.append(" ")
            valores_anterior.append(st.session_state.rutero_selected_saldo_anterior)
            columns_saldo_anterior = [
                "co_cli",
                "cli_des",
                "fec_emis",
                "fec_mid",
            ...
            df_saldo_anterior = DataFrame(
                [valores_anterior], columns=columns_saldo_anterior
            )
            mov_filtrados_x_rutero = concat(
                [df_saldo_anterior, mov_filtrados_x_rutero], axis=0, ignore_index=True
            )
            mov_filtrados_x_rutero["saldo final"] = mov_filtrados_x_rutero[
                "saldo"
            ].cumsum()
            ...
            mov_rut = st.dataframe(
                resumen_movimientos_style,
                column_config={
            ...
            with tb2_col5:
                if st.button("Reporte"):
                    ...
                    data = dict(
                        ...
                        fecha_i=datetime.strptime(fecha_ini, "%Y%m%d").strftime(
                            "%d-%m-%Y"
                        ),
                        fecha_f=datetime.strptime(fecha_fin, "%Y%m%d").strftime(
                            "%d-%m-%Y"
                        ),
                        total_ne="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_ne"].sum()
                        ),
                        total_fd="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_fd"].sum()
                        ),
                        total_fcp2="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_fcp2"].sum()
                        ),
                        total_ajust="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_ajust"].sum()
                        ),
                        total_cobro="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_cobro"].sum()
                        ),
                        total_ganancia="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_ganancia"].sum()
                        ),
                        total_fondo="{:,.2f}".format(
                            mov_filtrados_x_rutero["total_fondo"].sum()
                        ),
                    )
                    ...
                    mov_filtrados_x_rutero[columnas_a_formatear] = (
                        mov_filtrados_x_rutero[columnas_a_formatear].map(
                            lambda x: "{:,.2f}".format(x)
                        )
                    )
                    movimientos = mov_filtrados_x_rutero.to_dict("records")

                    file_html = rpt.reder_and_open_data(
                        encabezados=data, movimientos=movimientos, nombre_file=nombre
                    )
    ...
    with tab3:
        ...
        movimientos_ne = movimientos_nota_entrega_rutero(
            tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
        )
        if len(movimientos_ne) > 0:
            movimientos_ne = movimientos_ne[
                movimientos_ne["co_cli"] == st.session_state.rutero_selected
            ]
            ...
            movimientos_ne = movimientos_ne.style.format(
                {"total_ne": "{:,.2f}"}, precision=2
            )
            st.dataframe(
                movimientos_ne,
                column_config={
                    ...
                    "fec_emis": st.column_config.TextColumn("fecha", width="small"),
                },
                use_container_width=False,
                hide_index=True,
            )
    ...
    with tab4:
        ...
        mov_facturas = mov_facturas_directas(
            tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
        )[["co_cli", "cli_des", "doc_num", "fec_emis", "total_item"]]
        if len(mov_facturas) > 0:
            mov_facturas = mov_facturas[
                mov_facturas["co_cli"] == st.session_state.rutero_selected
            ]
            with tb4_col1:
                st.metric(
            ...
            mov_facturas = mov_facturas.style.format(
                {"total_item": "{:,.2f}"}, precision=2
            )
            st.dataframe(
                mov_facturas,
                column_config={
    ...
    with tab5:
        ...
        facturas_comercios_ruteros = mov_facturas_comercios(
            tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
        )
        if len(facturas_comercios_ruteros) > 0:
            facturas_comercios_ruteros = (
                facturas_comercios_ruteros.groupby(
                    ["co_cli_t1", "co_tran", "fec_emis", "cli_des", "doc_num_t1"],
                    sort=False,
                )
            ...
            facturas_comercios_ruteros.rename(
                columns={
                    "co_tran": "ruta",
                    "co_cli_t1": "co_cli",
                    "total_item_precio_2": "total_fact.",
                },
                inplace=True,
            )
            facturas_comercios_ruteros = facturas_comercios_ruteros[
                facturas_comercios_ruteros["ruta"] == st.session_state.rutero_selected
            ]
            with tb5_col1:
                st.metric(
                    label="Total facturación comercios.",
                    value="{:,.2f}".format(
                        facturas_comercios_ruteros["total_fact."].sum()
                    ),
                )
            with tb5_col2:
                st.metric(
            ...
            facturas_comercios_ruteros = facturas_comercios_ruteros.style.format(
                {"total_fact.": "{:,.2f}"}, precision=2
            )
            st.dataframe(
                facturas_comercios_ruteros,
                column_config={
    ...
    with tab6:
        ...
        ganancias_aplicadas = movimientos_ganancias_aplicadas(
            tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
        )[["co_cli", "co_tipo_doc", "fec_emis", "nro_doc", "nro_orig", "total_neto"]]
        if len(ganancias_aplicadas) > 0:
            ganancias_aplicadas = ganancias_aplicadas[
                ganancias_aplicadas["co_cli"] == st.session_state.rutero_selected
            ]
            st.metric(
                label="Total ganancias aplicadas",
                value="{:,.2f}".format(ganancias_aplicadas["total_neto"].sum()),
            )
            ganancias_aplicadas = ganancias_aplicadas.style.format(
                {"total_neto.": "{:,.2f}"}, precision=2
            )
            st.dataframe(
                ganancias_aplicadas,
                column_config={
    ...
    with tab7:
        ...
        ajustes_rutero = movimientos_ajustes(
            tip_cli="R", fecha_d=fecha_ini, fecha_h=fecha_fin
        )
        if len(ajustes_rutero) > 0:
            ajustes_rutero = ajustes_rutero[
                (ajustes_rutero["tip_cli"] == "R")
                & (~ajustes_rutero["co_tipo_doc"].isin(["IVAN", "AJPA", "AJNA"]))
                & (ajustes_rutero["co_cta_ingr_egr"] != "APS")
            ].copy()
            ajustes_rutero = (
                ajustes_rutero.groupby(
                    ["co_tipo_doc", "co_cli", "cli_des", "doc_num", "fec_emis"]
                )
                .agg({"total_neto": "sum"})
                .reset_index()
            )
            ...
            ajustes_rutero = ajustes_rutero[
                ajustes_rutero["co_cli"] == st.session_state.rutero_selected
            ]
            st.metric(
                label="Total ajustes",
                value="{:,.2f}".format(ajustes_rutero["total_ajust"].sum()),
            )
            ajustes_rutero = ajustes_rutero.style.format(
                {"total_ajust": "{:,.2f}"}, precision=2
            )
            st.dataframe(
                ajustes_rutero,
                column_config={
    ...
    with tab8:
        ...
        if len(cobros_rutero) > 0:
            ...
            cobros_rutero = (
                cobros_rutero.groupby(
                    ["co_cli", "cli_des", "co_tipo_doc", "cob_num", "fecha", "nro_doc"],
                    sort=False,
                )
                .agg({"cargo": "sum"})
                .reset_index()
            )
            cobros_rutero.rename(
                columns={"fecha": "fec_emis", "cargo": "total_cobro"}, inplace=True
            )
            cobros_rutero = cobros_rutero[
                cobros_rutero["co_cli"] == st.session_state.rutero_selected
            ]
            with tb8_col1:
                st.metric(
                    label="Total cobros",
            ...
            cobros_rutero = cobros_rutero.style.format(
                {"total_cobro": "{:,.2f}"}, precision=2
            )
            st.dataframe(
                cobros_rutero,
                column_config={
