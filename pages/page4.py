...
st.set_page_config(page_title="Procesos Masivos", layout="wide", page_icon="⚡")
...
with st.expander("mostrar"):
    ...
    if st.button("Actualizar Stock x almacen"):
        ...
        st.rerun()
st.caption(
    """Presione 'Actualizar' para recalcular el stock de artículos de inventario."""
)
...
with st.expander("mostrar"):
    ...
    with col1:
        fecha_ini = st.date_input("fecha inicial").strftime(
            "%Y%m%d %H:%M:%S"
    ...
    if st.button("Actualizar tasas facturas"):
        UpdateAll(conexion).agregar_info_tasa_facturas(
        ...
        st.rerun()
st.caption(
    """Presione 'Actualizar' para asignar la tasa de cambio de forma masiva a las facturas comprendidas entre el rango de fecha seleccionado."""
)

descrip_modulo = (
    " de Distribuidora Panaraca"
    if ClsEmpresa.modulo_seleccionado() == "DOEL"
    else " de Lácteos Doel"
)
