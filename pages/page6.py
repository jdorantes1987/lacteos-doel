...
st.set_page_config(page_title="Tributario", layout="wide", page_icon="⚡")
...
with st.expander("mostrar"):
    ...
    df_libro_ventas = libro_ventas(fecha_d=fecha_ini, fecha_h=fecha_fin)
    if df_libro_ventas is not None:
        df_libro_ventas.style.format("{:,.2f}")
        st.dataframe(
            df_libro_ventas,
        ...
        df_libro_ventas.to_excel(buf := BytesIO())
        st.download_button(
            "Descargar",
            buf.getvalue(),
...
with st.expander("mostrar"):
    ...
    with col1:
        ...
        mes, year = now.month, now.year
        current_fecha_ini = datetime(year, mes, 1, 0, 00, 00, 00000)
        fecha_ini = st.date_input("fecha inicial2", value=current_fecha_ini).strftime(
            "%Y%m%d %H:%M:%S"
        )  # Convierte la fecha a YYYYMMDD
    with col2:
        fecha_fin = st.date_input("fecha final2").strftime(
            "%Y%m%d 23:59:59"
        )  # Convierte la fecha a YYYYMMDD

    df_libro_compras = libro_compras(fecha_d=fecha_ini, fecha_h=fecha_fin)
    if df_libro_compras is not None:
        df_libro_compras.style.format("{:,.2f}")
        st.dataframe(
            df_libro_compras,
            use_container_width=True,
            hide_index=True,
        ...
        df_libro_compras.to_excel(buf := BytesIO())
        st.download_button(
            "Descargar",
            buf.getvalue(),
