...
st.set_page_config(page_title="CXC", layout="wide", page_icon="⚡")
...
st.caption(
    """Desplagar y luego hacer click en ':blue[Archivo de CXC]' para descargar achivo Excel de cuentas por cobrar a la fecha."""
)
with st.expander("mostrar"):
    tipo_cliente_select = st.selectbox(
        "Elije un :blue[tipo de cliente]:", ["Comercios", "Ruteros", "Todos"], 0
    )

    tipo_cliente = tipo_cliente_select[0]
    datos = cuentas_por_cobrar_pivot(tipo_cliente=tipo_cliente)
    saldo_cxc = datos.sort_values(by=["All"], ascending=False)
    cmap = plt.colormaps["summer"]
    st.dataframe(
        saldo_cxc.style.format("{:,.2f}").background_gradient(
